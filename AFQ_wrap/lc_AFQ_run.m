function [afq patient_data control_data norms abn abnTracts, excluded_subjects] = AFQ_run(sub_dirs, sub_group, afq)
...
for ii = runsubs
    ...
    if AFQ_get(afq, 'do tracking',ii) == 1
        ...
        catch  % If this subject's data is wrong, then go to nest subject
            [basedir, name] = fileparts(sub_dirs{ii});
            fprintf('Data of %s is wrong\n', name);
            excluded_subjects = cat(1, excluded_subjects, name);
            continue
        end
