function fH = lc_icatb_plot_connectogram(comp_network_names, varargin)
%% Functional network connectivity correlations (ROI-wise) are visualized as a connectogram plot.
% This function is modified from GIFT software:: icatb_plot_connectogram.m
% Difference with original function, icatb_plot_connectogram, is that the param_file 'image_file_names'
% Users should cite the GIFT software.
% Inputs:
%
...
% a. lc_icatb_plot_connectogram(comp_network_names, 'C', C, 'threshold', 1, 'image_file_names', tmp4dnii, 'colorbar_label', 'Corr', 'comp_labels', comp_labels, 'line_width', 0.2, 'display_type', 'render');
% b. lc_icatb_plot_connectogram(comp_network_names, 'C', C, 'threshold', 1.5, 'image_file_names', fname, 'colorbar_label', 'Corr', 'display_type', 'render');
%
...
if (~exist('I', 'var'))
    ...
    elseif (strcmpi(display_type, 'render'))
        ...
        disp('Do not display components');
...
for n = 1:numSlices
    ...
    textDeg = th_deg(n) + 0;  % Revised by Li Chao from textDeg = th_deg(n) + 90;
    textDeg = rem(textDeg + 3600, 360);
    if (textDeg > 80 && textDeg < 270)
        textDeg = textDeg + 180;
    end
    % Revised By Lichao (UI_FS - 5)
    thA = text(mpX(n), mpY(n), compNames{n}, 'parent', aH, 'horizontalalignment','center','rotation',textDeg,'fontsize', UI_FS - 6, 'fontweight', 'bold', 'color', comp_txt_color, ...
