function [orignalALFF, ALFF, frequentVector, uniqueFrequencyDomainSignal]...
            =lc_alff(AllVolume, repeat_time, nDimTimePoints, LowCutoff, HighCutoff, CUTNUMBER)
% LC_ALFF
% INPUT:
% repeat_time: TR repeat time
% output:
    % ALFF:2D ALFF value
    ...
    if (LowCutoff >= sampleFreq/2) % All high included
        ...
        idx_LowCutoff = ceil(LowCutoff * paddedLength * repeat_time + 1);
        % Change from round to ceil: idx_LowCutoff = round(LowCutoff *paddedLength *repeat_time + 1);
    end
    if (HighCutoff>=sampleFreq/2)||(HighCutoff==0) % All low pass
        idx_HighCutoff = paddedLength/2 + 1;
    else % Low pass, such as freq < 0.08 Hz
        idx_HighCutoff = fix(HighCutoff *paddedLength *repeat_time + 1);
        % Change from round to fix: idx_HighCutoff	=round(HighCutoff *paddedLength *repeat_time + 1);
    end
