...
is_pca = 1;  
explained_cov = 0.95; 
min_clustering_solutions = 2; 
max_clustering_solutions = 10; 
cvfold = 10; 
...
if ~strcmp(mask_file, '')
    if strcmp(feature_type, 'fc')
        % If the feature is functional connectivity, then extract the upper triangle matrix
...
if (~ isempty(patient_cov)) && (length(patient_cov) ~= length(num_patient))
    error('Number of covariates is not match number of patients!');
    return;
end

if ~ isempty(hc_cov) && (length(hc_cov) ~= length(num_hc))
...
%% ------------added by Li Chao; 2020/06/17---------------
nan = isnan(M);
inf = isinf(M);
if (sum(nan(:)) ~= 0) || (sum(inf(:)) ~= 0)
    save('D:\WorkStation_2018\Workstation_2020_transdiagnositic_subtyping\Data\M.mat', 'M');
    % fill missing data
    M(isinf(M)) = NaN;
    x = 1:size(M,2);
    [M,~] = fillmissing(M','linear','SamplePoints',x);
    M = M';
end
%% --------------------------------------------------------   
