...
class SVCRFECV():
    ...
    def main_svc_rfe_cv(sel):
        ...
        for i in range(n_site):
            ...
            # if sel.is_dim_reduction:
            #     sel.coef.append(model_dim_reduction.inverse_transform(clf.coef_))  # save coef
            # else:
    ...
    def training(sel, train_X, train_y, cv):
        ...
        range_dimreduction = np.linspace(0.1, 0.99, 20).reshape(20,)
        range_k = np.int32(range_dimreduction * train_X.shape[1])
        ...
        model = RandomizedSearchCV(
                pipe, n_jobs=1, param_distributions=param_grid, cv=cv, 
